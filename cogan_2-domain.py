...
for epoch in range(opt.n_epochs):
    for i, ((imgs1, _), (imgs2, _)) in enumerate(zip(dataloader1, dataloader2)):
        ...
        # print(imgs1.shape)
        # print(imgs2.shape)
        imgs2 = Variable(imgs2[:, 0, :, :].unsqueeze(1).type(Tensor))#.expand(imgs1.size(0), 1, opt.img_size, opt.img_size))
        # print(imgs2.shape)
        # ------------------
        #  Train Generators
        # ------------------
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(gen_imgs, "images_2_domain/%d.png" % batches_done, nrow=8, normalize=True)
