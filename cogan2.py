...
def weights_init_normal(m):
    ...
    #     torch.nn.init.normal_(m.weight, 0.0, 0.02) #m.weight.data --> m.weight
...
class CoupledGenerators(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.shared_conv1 = nn.Sequential(
            nn.ConvTranspose2d(latent_dim, 1024, kernel_size=4, stride=1), #opt.latent_dim --> 128
    ...
    def forward(self, noise):
        ...
        out = noise.view(-1, self.latent_dim, 1,1)#self.init_size, self.init_size)
...
class CoupledDiscriminators(nn.Module):
    def __init__(self):
        ...
        self.D1 = nn.Sequential(
            nn.ConvTranspose2d(1, 20, kernel_size=5, stride=1, padding=0),
        ...
        self.shared_conv = nn.Sequential(
            nn.ConvTranspose2d(20, 50, kernel_size=5, stride=1),
            nn.MaxPool2d(kernel_size=2),
            nn.Flatten(),
            nn.Linear(5000,500), 
            nn.PReLU()
    ...
    def forward(self, x_a, x_b):
        ...
        out = self.shared_conv(h0_a)
        # out = out.view(out.shape[0], -1)
        validity1 = self.fcD1(out)
        # Determine validity of second image
        out = self.shared_conv(h0_b)
        # out = out.view(out.shape[0], -1)
        validity2 = self.fcD2(out)
        # print(h1.shape)
        # h1_f = h1.view(h1.shape[0], -1)#self.flatten(h1)
        # # # print(h1_f.shape)
        # h2 = self.fc1(h1_f)
        # # print(h2.shape)
        # validity1 = self.fcD1(h2)
        # validity2 = self.fcD2(h2)
...
# Initialize models
coupled_generators = CoupledGenerators(latent_dim=opt.latent_dim)
...
for epoch in range(opt.n_epochs):
    for i, ((imgs1, _), (imgs2, _)) in enumerate(zip(dataloader1, dataloader1)):
        ...
        if batches_done % opt.sample_interval == 0:
            ...
            save_image(gen_imgs, "images_original/%d.png" % batches_done, nrow=8, normalize=True)
